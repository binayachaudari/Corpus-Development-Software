...
router.route('/').get(require('../../middleware/validateToken'), require('../../controller/users/users').getUser);

router
  .route('/login')
  .post(
    [check('email', 'Invalid Email').isEmail(), check('password', 'Password is required').notEmpty()],
    validation,
    authLimiter,
    login
  );
...
router
  ...
  .post([check('email', 'Invalid Email').isEmail()], validation, forgetLimiter, password.forgotPassword);
...
router
  ...
  .patch(
    ...
    validation,
    authenticateToken,
    password.resetDefaultPassword
  );

router
  .route('/reset-password/:token')
  .patch([check('password', 'Password must be atleast 8 characters').isLength({ min: 8 })], password.resetPassword);
