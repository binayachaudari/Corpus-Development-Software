...
router
  ...
  .post(
    ...
    validation,
    authenticateToken,
    checkRole.restrictTo('Admin', 'Developer'),
    isValidAssignment,
    reviewTask.addReviewTask
  );
...
router.route('/assignments').get(authenticateToken, checkRole.restrictTo('Reviewer'), getFileDetails.getMyFiles);

router
  .route('/assignments/file-details/:file_id')
  .get(authenticateToken, checkRole.restrictTo('Admin', 'Developer', 'Reviewer'), getFileDetails.detailsOfFileID);

router
  .route('/assignments/:file_id')
...
router.route('/assignments/:file_id/:index').get(authenticateToken, checkRole.restrictTo('Reviewer'));
