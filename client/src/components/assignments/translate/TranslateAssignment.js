...
export const getColorType = (status) => {
  return status === 'assigned'
    ? 'primary'
    : status === 'under_translation' || status === 'under_review'
    ? 'secondary'
    : 'success';
};
                  ...
                  <Card key={index} border={getColorType(item.status)} {...slideInAnimationWithIndex(index, 0.4)}>
                    ...
                    <Card.Body>
                      ...
                      <Card.Text>
                        ...
                        <br />
                        {item.tamang_filename && (
                          <>
                            <b>Tamang Filename:</b> {item.tamang_filename}
                            <br />
                          </>
                        )}
                        {item.nepali_filename && (
                          <>
                            {' '}
                        ...
                        {!item.tamang_filename && !item.nepali_filename ? (
                          <>
                            {' '}
                            <b>Source File</b> {item.file_details.filename}
                            <br />
                          </>
                        ) : (
                          ''
                        ...
                        <b># of Sentences:</b>{' '}
                        {item.file_details && item.file_details.end_index - item.file_details.start_index + 1}
                        <br />
